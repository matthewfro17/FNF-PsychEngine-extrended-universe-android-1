...
function onCreatePost()
  makeChar('none', 'none', 300, 0, 'none note') --tag, char, x, y, notetype for singing
  setProperty('none.alpha', 0.5) --you can setProperty stuff too
  addChar('none', true)
  if a then
    a()
  end
...
function onStepHit()	
    if curStep == 0 then
        makeChar('none', 'none', -200, 0, 'none note') --tag, char, x, y, notetype for singing
        addChar('none', true)
    end
	if curStep == 672 then
		removeChar('none', true)
        makeChar('cakebi', 'cakebi', -200, 0, 'cakebi note') --tag, char, x, y, notetype for singing
        addChar('cakebi', true)
	end
	if curStep == 943 then
		removeChar('cakebi', true)
        makeChar('nuggbi', 'nuggbi', -200, 0, 'nuggbi note') --tag, char, x, y, notetype for singing
        addChar('nuggbi', true)
	end
	if curStep == 1215 then
		removeChar('nuggbi', true)  
        makeChar('none', 'none', -200, 0, 'none note') --tag, char, x, y, notetype for singing
        addChar('none', true)
    end
	...
	if curStep == 3631 then
		removeChar('none', true)  
        makeChar('baggett', 'baggett', -200, 0, 'baggett note') --tag, char, x, y, notetype for singing
        addChar('baggett', true)
	end
	if curStep == 3775 then
		removeChar('baggett', true) 
		removeChar('cakebi', true)
		removeChar('nuggbi', true)  
        makeChar('cakebi', 'cakebi', -200, 0, 'cakebi note') --tag, char, x, y, notetype for singing
        addChar('cakebi', true)
